...
#
# Specify job name.
#SBATCH --job-name=overfit_mv
#
# Specify output file.
#SBATCH --output=overfit_mv_%j.log
#
# Specify error file.
#SBATCH --error=overfit_mv_%j.err
#
...
python distillation_test_multi_view_chat.py \
  --epochs 2000 \
  --num_workers 1 \
  --print_freq 500 \
  --save_freq 250 \
  ...
  --amp \
  --use_wandb \
  --wandb_name "overfit_single_scene_mv" \
  --disable_scheduler \
  --weight_decay 0 \
  --multi_view_mode
