...
class MultiViewDistillationDataset(Dataset):
    """
    Dataset multi-view: ogni sample è una scena (sottocartella) che contiene N viste (immagini).
    In modalità precomputed, ci si aspetta un file .pt per ciascuna view nel mirror folder delle features.
    """
    def __init__(
        self,
        scenes_root: str,
        ...
        if self.features_root is None and self.teacher_extractor is None:
            raise ValueError("Either features_root or teacher_extractor must be provided")

        self.mode = "precomputed" if self.features_root else "online"

        # individua le scene (sottocartelle)
        if scene_dirs is None:
            self.scene_dirs = sorted([
                str(self.scenes_root / d)
                for d in os.listdir(self.scenes_root)
                if (self.scenes_root / d).is_dir()
            ])
        else:
            self.scene_dirs = scene_dirs

        print(f"[Dataset-MV] Mode: {self.mode.upper()}")
        print(f"[Dataset-MV] Loaded {len(self.scene_dirs)} scenes from {scenes_root}")
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if len(view_paths) == 0:
            raise RuntimeError(f"No images found in scene dir: {scene_dir}")

        if self.mode == "precomputed":
            # Carica teacher features per ciascuna view
            teacher_feats_list = []
            for vp in view_paths:
                img_name = Path(vp).stem
                # le features sono in una cartella mirror: features_root/<scene_name>/<img_name>.pt
                scene_name = scene_dir.name
                feat_path = self.features_root / scene_name / f"{img_name}.pt"
                if not feat_path.exists():
                    raise FileNotFoundError(f"Teacher features not found: {feat_path}")
                tf = torch.load(feat_path, map_location="cpu", weights_only=False)
                if tf.ndim == 4 and tf.shape[0] == 1:
                    tf = tf.squeeze(0)
                teacher_feats_list.append(tf)
            # impila sulle view -> (V, C, H, W)
            teacher_feats = torch.stack(teacher_feats_list, dim=0)
            return {
                "scene_dir": str(scene_dir),
                "view_paths": view_paths,
                "teacher_features": teacher_feats,  # (V, C, H, W)
                "pil_images": None,
            }
        else:
            # Online: rimanda la creazione di teacher features alla collate/train
            from PIL import Image
            pil_images = []
            for vp in view_paths:
                pil_images.append(Image.open(vp).convert("RGB"))
            return {
                "scene_dir": str(scene_dir),
                "view_paths": view_paths,
                "teacher_features": None,  # calcolate online
                "pil_images": pil_images,  # lista di PIL per view
            }

def collate_fn_distillation_mv(batch: List[Dict]) -> Dict:
    """
    Collate per multi-view.
    Con batch_size=1 restituisce una singola scena, ma supporta batch>1 mantenendo liste.
    """
    scene_dirs = [item["scene_dir"] for item in batch]
    view_paths_per_scene = [item["view_paths"] for item in batch]

    if batch[0]["teacher_features"] is not None:
        # Precomputed: stack per scena, poi lista su batch
        teacher_features_per_scene = [item["teacher_features"] for item in batch]  # ciascuno (V, C, H, W)
    else:
        teacher_features_per_scene = None

    pil_images_per_scene = [item["pil_images"] for item in batch] if batch[0]["pil_images"] is not None else None

    return {
        "scene_dirs": scene_dirs,
        "view_paths_per_scene": view_paths_per_scene,  # List[List[str]]
        "teacher_features_per_scene": teacher_features_per_scene,  # List[Tensor] or None
        "pil_images_per_scene": pil_images_per_scene,  # List[List[PIL.Image]] or None
    }
...
def build_distillation_dataloader(
    ...
    """
    Se multi_view_mode=True, image_dir contiene sottocartelle (scene) e si usa MultiViewDistillationDataset.
    Altrimenti si usa DistillationDataset classico single-view.
    """
    if multi_view_mode:
        dataset = MultiViewDistillationDataset(
            scenes_root=image_dir,
            features_root=features_dir,
            teacher_extractor=teacher_extractor,
            scene_dirs=None,  # opzionale: filtri scena se necessario
        )
        collate_fn = collate_fn_distillation_mv
    else:
...
def forward_pass_scene_multi_view(
    model: torch.nn.Module,
    view_paths: List[str],
    device: torch.device,
    use_amp: bool,
    ...
    """
    Esegue forward passando tutte le view della scena insieme per attivare cross-attention.
    Ritorna (V, C, H, W) con V = numero di view.
    """
    return forward_pass_distillation_unified(
        model=model,
        image_paths=view_paths,
        device=device,
        use_amp=use_amp,
        amp_dtype=amp_dtype,
        process_individually=False,  # multi-view -> insieme
    )
...
def train_one_epoch_distillation(
    ...
    Supporta single-view (default) e multi-view (args.multi_view_mode=True).
    In multi-view:
      - il DataLoader fornisce scene -> liste di view per scena
      - lo studente riceve tutte le view insieme (cross-attention)
      - il teacher online rimane per-view (una immagine per volta)
      - la loss è per-view (batch = numero di view)
    """
    model.train(True)
    ...
    micro_step = 0  # conta i passi effettivi (scene singole in MV, batch in SV)
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        if args.multi_view_mode:
            # ------- MULTI-VIEW PATH -------
            if mode is None:
                mode = "precomputed" if batch.get("teacher_features_per_scene") is not None else "online"
                print(f"[Train] (MV) Feature extraction mode: {mode.upper()}")

            view_paths_per_scene = batch["view_paths_per_scene"]  # List[List[str]]

            # Supporta batch di più scene iterando sulle scene
            for scene_idx in range(len(view_paths_per_scene)):
                view_paths = view_paths_per_scene[scene_idx]

                # Teacher features: precomputed o online (per-view)
                if mode == "precomputed":
                    teacher_features = batch["teacher_features_per_scene"][scene_idx].to(device, non_blocking=True)  # (V,C,H,W)
                else:
                    if teacher_extractor is None:
                        raise ValueError("teacher_extractor required for online mode but not provided")
                    pil_images_scene = batch["pil_images_per_scene"][scene_idx]
                    with torch.no_grad():
                        teacher_features = teacher_extractor(pil_images_scene).to(device, non_blocking=True)  # (V,C,H,W)

                # Student forward con tutte le view per attivare cross-attention
                student_features = forward_pass_scene_multi_view(
                    model=model,
                    view_paths=view_paths,
                    device=device,
                    use_amp=args.amp,
                ...
                # Allinea la risoluzione se necessario
                if student_features.shape[-2:] != teacher_features.shape[-2:]:
                    H, W = teacher_features.shape[-2:]
                    student_features = F.interpolate(student_features, size=(H, W), mode="bilinear", align_corners=False)
                ...
                try:
                    ...
                    md = float(md); sd = float(sd); cs = float(cs)
                except Exception:
                    md = sd = 0.0; cs = cos_sim_value

                if not math.isfinite(loss_value):
                    print(f"Loss is {loss_value}, stopping training", flush=True)
                    sys.exit(1)

                # W&B logging (per scena, a intervalli)
                is_main_process = (train_tools.get_rank() == 0)
                if (
                    args.use_wandb and WANDB_AVAILABLE and is_main_process
                    and (wandb is not None) and (getattr(wandb, "run", None) is not None)
                ):
                    if micro_step % getattr(args, "log_freq", 100) == 0:
                        wandb.log(
                            {
                ...
                # Accumulation, backward, step
                loss = loss / accum_iter
                loss.backward()
                micro_step += 1

                if micro_step % accum_iter == 0:
                    if args.clip_grad > 0:
                        torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip_grad)
                    optimizer.step()
                    optimizer.zero_grad()

                # Aggregazione metriche pesate per numero di view
                batch_size_views = int(student_features.shape[0])
                total_samples += batch_size_views
                sum_loss += loss_value * batch_size_views
                sum_mse += mse_value * batch_size_views
                sum_cos += cos_value * batch_size_views
                sum_cos_sim += cos_sim_value * batch_size_views
                sum_mean_diff += md * batch_size_views
                sum_std_diff += sd * batch_size_views
            ...
            continue  # prossima batch del dataloader (multi-view già gestito qui)
        ...
        student_features = forward_pass_distillation_unified(
            ...
            use_amp=args.amp,
            amp_dtype=args.amp_dtype,
            process_individually=True,  # single-view: no cross-attention
        )
        ...
        batch_size = int(student_features.shape[0])
...
def validate_one_epoch_distillation(
    ...
    Supporta single-view e multi-view (args.multi_view_mode=True).
    """
    model.eval()
    metric_logger = train_tools.MetricLogger(delimiter=" | ")
    ...
    for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        if args.multi_view_mode:
            ...
            if mode is None:
                mode = "precomputed" if batch.get("teacher_features_per_scene") is not None else "online"
                print(f"[Val] (MV) Feature extraction mode: {mode.upper()}")

            view_paths_per_scene = batch["view_paths_per_scene"]
            ...
            for scene_idx in range(len(view_paths_per_scene)):
                ...
                # Visualizzazioni: solo prima scena del primo batch
                if args.save_visualizations and batch_idx == 0 and scene_idx == 0:
                    save_pca_visualizations(
                        student_features=student_features,
                        teacher_features=teacher_features,
                        image_paths=view_paths,  # lista delle view della scena
                        epoch=epoch,
                        output_dir=args.output_dir,
                    )

                # Aggrega per numero di view
                batch_size_views = int(student_features.shape[0])
                total_samples += batch_size_views
                sum_loss += loss_value * batch_size_views
                sum_mse += mse_value * batch_size_views
            ...
            continue  # prossima batch (multi-view già gestito)
        ...
        if mode == "precomputed":
            ...
            teacher_features = teacher_extractor(batch["pil_images"]).to(device, non_blocking=True)
...
def get_args_parser():
    ...
    # python distillation_test_multi_view_chat.py --epochs 10 --log_freq 1 --save_freq 1 --save_visualizations --multi_view_mode --disable_scheduler
